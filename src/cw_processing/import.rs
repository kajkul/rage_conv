...
pub async fn xml(file_path: PathBuf) {
    ...
    let res = tokio::task::spawn_blocking(move || {
        ...
        unsafe { ffi::cw_import_xml(c_path.as_ptr() as *const u8) }
    })
    .await
    ...
    let slice = unsafe { std::slice::from_raw_parts(res.data, res.data_len) };
    let data = unsafe { LibcFreer::new(slice, res.data as *mut c_void) };

    if let Ok(file_name) = unsafe { CStr::from_ptr(res.file_name as *const i8).to_str() } {
        let file_name = file_name.to_owned();
        unsafe { libc::free(res.file_name as *mut libc::c_void) };
